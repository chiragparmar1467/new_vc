...
class Collection extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        $table_data = $this->db->query('SELECT vc.id as vc_id,vc.vc_opening_date,vc.vc_closing_date,vc.name,vc.status,grp.id as group_id,grp.name as group_name
        ,grp.short_name,grp.start_date,grp.end_date 
        FROM vc_master as vc 
        INNER JOIN group_master as grp ON vc.group_id = grp.id 
        where vc.fk_financial_year_id = '.$_SESSION['year'].' AND vc.deleted = 0 AND vc.status = 1 AND grp.status = 1 AND grp.deleted = 0
        order by vc.id DESC
        ;')->result_array();
    ...
    public function form_view($coll_id = NULL){
        ...
        $this->data['table_data'] = $this->db->query('select acc.account_no,
                                                             coll.id as coll_id,
                                                             ...
                                                             DATE_FORMAT(grp.start_date, "%d-%m-%Y") as start_date,
                                                             grp.id as grp_id,
                                                             grp.status as group_status, 
                                                             DATE_FORMAT(grp.end_date, "%d-%m-%Y") as end_date,
                                                             vc.name,
                                                             pay.payment_mode,
                                                             pay.id from collection_master as coll 
        INNER JOIN member_master as mem ON coll.fk_member_id = mem.id 
