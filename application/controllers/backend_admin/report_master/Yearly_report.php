...
class Yearly_report extends Admin_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->controllerPath = 'backend_admin/report_master/Yearly_report/';
        $this->viewPath = 'admin/yearly_report/';
        $this->uploadFolder = '';
    ...
    public function create()
    {
        ...
        if(isset($_POST['submit'])){
            ...
            $account_no = $this->input->post('acc_no');
                 ...
                 if(!empty($year) || !empty($member_name) || 
                    ...
                    if(!empty($account_no)){
                        ...
                        $ac = " Account Number = $account_no";
                         ...
                         $collection_query = ("(SELECT mem.member_name ,rvc.fk_acc_id,rvc.collection_date ,rvc.fk_vc_id, rvc.bank_name , rvc.cheque_no ,
                         SUM(rvc.amount) AS payed_amount,NULL as collected_amount, vc.name as vc_name, vc.vc_opening_date,vc.vc_closing_date,vc.minimum_amount,vc.maximum_amount,vc.instalment,
                         grp.name,grp.start_date as grp_start_date,grp.end_date,pay.payment_mode
                         ...
                         WHERE rvc.status = 1 AND rvc.deleted = 0  AND rvc.fk_financial_year_id = ".$year .  $filter_date_for_return . $grp_no_for_return . $vc_no_for_return . $member_for_return . $acc_no_for_return .")
                    UNION ALL
                        (SELECT mem.member_name ,coll.fk_acc_id,coll.collection_date ,coll.fk_vc_id, coll.bank_name , coll.cheque_no, NULL AS payed_amount,
                        SUM(coll.amount) as collected_amount, vc.name as vc_name, vc.vc_opening_date,vc.vc_closing_date,vc.minimum_amount,vc.maximum_amount,vc.instalment,
                       grp.name,grp.start_date as grp_start_date ,grp.end_date ,pay.payment_mode
                       FROM member_master as mem
                       ...
                       WHERE coll.deleted = 0 AND coll.fk_financial_year_id = ".$year. " AND coll.status = 1 " .$filter_date_for_collection.$grp_no_for_collection.$vc_no_for_collection.$member_for_collection.$acc_no_for_collection. " order by coll.fk_vc_id DESC);"); 
                       
                      
                       $data['collection_data'] = $this->db->query($collection_query)->result_array();
    
                         $credit_debit_mst = ("(SELECT CM.id, CM.money_credit_date AS collection_date,
                         SUM(CM.amount) AS credit_amount,NULL AS debit_amount,
                         MM.member_name, AM.account_no, CM.bank_name, CM.cheque_no, CM.credit_bank_name,
                         PM.payment_mode,'Credit' AS type, PM.id AS payment_id 
                         ...
                         JOIN payment_mode PM ON PM.id = CM.fk_payment_id
                         WHERE CM.status = 1 AND CM.deleted = 0 AND CM.fk_financial_year_id = ".$year .$mem_for_cr.$acc_for_cr. ")
                         UNION
                         (SELECT DM.id, DM.debit_money_date AS collection_date, NULL AS credit_amount, 
                         SUM(DM.amount) AS debit_amount,
                         MM.member_name, AM.account_no, DM.bank_name, DM.cheque_no, DM.credit_bank_name,
                         PM.payment_mode,'Debit' AS type , PM.id AS payment_id
                         ...
                         JOIN payment_mode PM ON PM.id = DM.fk_payment_id
                         WHERE DM.status = 1 AND DM.deleted = 0 AND DM.fk_financial_year_id = ".$year .$mem_for_db.$acc_for_db. ");"); 
