...
class Model_groups extends CI_Model
{
	...
	{
		if ($groupId) {
			$sql = "SELECT * FROM groups WHERE id = ?  AND deleted = 0";
			$query = $this->db->query($sql, array($groupId));
			return $query->row_array();
		}

		$sql = "SELECT * FROM groups WHERE id != ?  AND deleted = 0 ORDER BY id DESC";
	...
	public function delete($id)
	{
		...
		$delete = $this->db->query("UPDATE groups SET deleted = 1 WHERE id = $id");
